...
export default function WorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (/401|403|unauthorized|authorization|Invalid or missing/i.test(msg)) {
  ...
  return (
    <main className="flex h-screen flex-col bg-(--color-bg) text-(--color-text)">
      ...
      {error ? (
        <div className="flex flex-1 items-center justify-center bg-(--color-bg)">
        ...
        <div className="flex flex-1 items-center justify-center bg-(--color-bg)">
          <div className="flex flex-col items-center gap-3">
            <svg className="h-8 w-8 animate-spin text-(--color-primary)" viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
            </svg>
